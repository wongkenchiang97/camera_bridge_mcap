...
class Sink final:public bridge::IFrameConsumer{public:
 ...
 void onRightFrame(const bridge::RightFrameEvent&e)override{right=e;rightDeviceTimestamps.push_back(e.device_timestamp_us);rights++;}
 ...
 void onCameraCalibration(const bridge::CameraCalibrationEvent&e)override{calibration=e;if(e.has_color&&!e.has_right)colorCalibrations++;if(e.has_right)rightCalibrations++;calibrations++;}
 bridge::ColorFrameEvent color;bridge::RightFrameEvent right;bridge::DepthFrameEvent depth;bridge::ImuSampleEvent imu;bridge::CameraCalibrationEvent calibration;bridge::ExtrinsicsEvent extrinsics;std::vector<uint32_t>colorSources;std::vector<uint64_t>colorDeviceTimestamps,rightDeviceTimestamps,depthDeviceTimestamps,imuDeviceTimestamps;int colors=0,rights=0,depths=0,imus=0,calibrations=0,colorCalibrations=0,rightCalibrations=0,extrinsicEvents=0;
};
int main(){namespace fs=std::filesystem;using namespace camera_bridge_mcap;auto path=fs::temp_directory_path()/"camera_bridge_mcap_roundtrip.mcap";std::error_code ec;fs::remove(path,ec);
 ...
 bridge::CameraCalibrationEvent cal;cal.source_id=7;cal.timestamp_us=1000000;cal.has_color=true;cal.color_intrinsic={2,2,100,101,1,1};cal.has_right=true;cal.right_intrinsic={2,2,102,103,1,1};recorder.onCameraCalibration(cal);
 ...
 bridge::RightFrameEvent right;right.source_id=7;right.timestamp_us=1000120;right.device_timestamp_us=558;right.bgr=color.bgr.clone();right.bgr.at<cv::Vec3b>(0,0)[0]=99;recorder.onRightFrame(right);
 ...
 if(sink.calibration.source_id!=7||sink.colorCalibrations!=1||sink.rightCalibrations!=1)return 9;if(sink.colors!=2||sink.rights!=1||sink.depths!=1||sink.imus!=1||sink.calibrations!=2||sink.extrinsicEvents!=1)return 3;if(sink.colorSources!=std::vector<uint32_t>({7,8})||sink.colorDeviceTimestamps!=std::vector<uint64_t>({555,655})||sink.rightDeviceTimestamps!=std::vector<uint64_t>({558})||sink.depthDeviceTimestamps!=std::vector<uint64_t>({556})||sink.imuDeviceTimestamps!=std::vector<uint64_t>({557})||sink.color.frame_id!=bridge::cameraColorOpticalFrame(8)||sink.right.frame_id!=bridge::cameraRightOpticalFrame(7))return 4;if(cv::norm(sink.color.bgr,color.bgr,cv::NORM_INF)!=0||cv::norm(sink.right.bgr,right.bgr,cv::NORM_INF)!=0||cv::norm(sink.depth.depth_mono16,depth.depth_mono16,cv::NORM_INF)!=0)return 5;if(sink.imu.accel.x!=1||sink.imu.gyro.z!=6||sink.imu.frame_id!="camera7_imu_frame")return 6;if(sink.extrinsics.transforms.size()!=1||std::abs(sink.extrinsics.transforms[0].extrinsic.trans[0]-.1f)>1e-6f)return 7;if(bridge::cameraTopicPrefix(12)!="/camera12"||bridge::cameraDepthOpticalFrame(12)!="camera12_depth_optical_frame"||bridge::cameraRightOpticalFrame(12)!="camera12_right_optical_frame")return 8;fs::remove(path,ec);std::cout<<"MCAP round trip passed\n";return 0;}
