...
class Sink final:public bridge::IFrameConsumer{public:
 void onColorFrame(const bridge::ColorFrameEvent&e)override{color=e;colorSources.push_back(e.source_id);colors++;}
 void onDepthFrame(const bridge::DepthFrameEvent&e)override{depth=e;depths++;}
 void onImuSample(const bridge::ImuSampleEvent&e)override{imu=e;imus++;}
 ...
 bridge::ColorFrameEvent color;bridge::DepthFrameEvent depth;bridge::ImuSampleEvent imu;bridge::CameraCalibrationEvent calibration;bridge::ExtrinsicsEvent extrinsics;std::vector<uint32_t>colorSources;int colors=0,depths=0,imus=0,calibrations=0,extrinsicEvents=0;
};
int main(){namespace fs=std::filesystem;using namespace camera_bridge_mcap;auto path=fs::temp_directory_path()/"camera_bridge_mcap_roundtrip.mcap";std::error_code ec;fs::remove(path,ec);
 ...
 bridge::DepthFrameEvent depth;depth.source_id=7;depth.timestamp_us=1000200;depth.device_timestamp_us=556;depth.depth_mono16=cv::Mat(2,2,CV_16UC1);depth.depth_mono16.at<uint16_t>(0,0)=1234;depth.depth_mono16.at<uint16_t>(0,1)=2345;depth.depth_mono16.at<uint16_t>(1,0)=3456;depth.depth_mono16.at<uint16_t>(1,1)=4567;recorder.onDepthFrame(depth);
 bridge::ImuSampleEvent imu;imu.source_id=7;imu.timestamp_us=1000250;imu.device_timestamp_us=557;imu.has_accel=imu.has_gyro=true;imu.accel={1,2,3};imu.gyro={4,5,6};recorder.onImuSample(imu);recorder.stop();
 ...
 if(sink.calibration.source_id!=7||sink.calibration.color_frame_id!="camera7_color_optical_frame")return 9;if(sink.colors!=2||sink.depths!=1||sink.imus!=1||sink.calibrations!=1||sink.extrinsicEvents!=1)return 3;if(sink.colorSources!=std::vector<uint32_t>({7,8})||sink.color.device_timestamp_us!=655||sink.color.frame_id!=bridge::cameraColorOpticalFrame(8))return 4;if(cv::norm(sink.color.bgr,color.bgr,cv::NORM_INF)!=0||cv::norm(sink.depth.depth_mono16,depth.depth_mono16,cv::NORM_INF)!=0)return 5;if(sink.imu.accel.x!=1||sink.imu.gyro.z!=6||sink.imu.frame_id!="camera7_imu_frame")return 6;if(sink.extrinsics.transforms.size()!=1||std::abs(sink.extrinsics.transforms[0].extrinsic.trans[0]-.1f)>1e-6f)return 7;if(bridge::cameraTopicPrefix(12)!="/camera12"||bridge::cameraDepthOpticalFrame(12)!="camera12_depth_optical_frame")return 8;fs::remove(path,ec);std::cout<<"MCAP round trip passed\n";return 0;}
